...
- **#56 — Mode standalone (ZIP autonome stagiaire)** — **À FAIRE, insérée AVANT le module 3.** Fournir un ZIP hors-ligne complet (site de cours servi en local via `jwebserver`, JDK + git portables embarqués, moulinette pré-compilée, launcher Windows unique). Brief : [`superpowers/2026-06-02-standalone-kickoff.md`](superpowers/2026-06-02-standalone-kickoff.md). Décisions figées : site statique servi par `jwebserver` (zéro Node runtime) ; Maven/`.m2` hors scope (la moulinette grade via javac + JUnit ConsoleLauncher) ; Windows d'abord ; git portable embarqué. Acquis : `scripts/build-bundle.sh`/`.ps1`. Doc des **deux modes** (dépôt / standalone) posée dans `README.md` + `docs/piscine-stagiaire.md`.
