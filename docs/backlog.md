...
6. **#14 — `CONTRIBUTING.md` + `LICENSE`.** Cadrage institutionnel : comment ajouter exos/chapitres, droits d'usage. Peu coûteux.
...
**Statut** : **#11a Faite** (commit `8e3399b`, [PR #1](https://github.com/Benjamin-Curlier/piscine-java/pull/1)) — **#11b Faite** (job `valider-solutions` via `scripts/valider-solutions.sh` : `mvn test` sur chaque `solution/`)
...
**Statut** : Faite — branche `feature/contributing-license` (PR à merger). LICENSE = en-tête provisoire « licence MIT » (arbitrage définitif mainteneurs du projet à venir).
...
**Pourquoi** : projet institutionnel partagé entre formateurs — cadrer l'ajout d'exos/chapitres et clarifier les droits d'usage avant d'élargir la base de contributeurs.
...
- [x] `LICENSE` : en-tête provisoire « licence MIT » posé en attendant l'arbitrage mainteneurs du projet.
...
> Spec validée (archivée hors dépôt).
> Tous ces items partagent le pré-requis #9 (squelette moulinette).
> **Itération réalisée** sur la branche `feature/mvp-console-correction` — plan d'implémentation archivé hors dépôt.
...
> Spec et plan archivés hors dépôt.
...
**Statut** : Faite — branche `feature/m1-chapitres` (PR à merger). Spec et plan archivés hors dépôt.
**Priorité** : Haute
...
**Statut** : Faite — livrée en 3 PRs (un sous-groupe par PR). Spec et plan archivés hors dépôt. 1.1 (PR #6) et 1.2 (PR #7) mergées ; 1.3 en PR finale.
...
- **#18 — Module 2** (5 chapitres, 12 exercices, 4 sous-groupes) — **FAITE** (12/12 exercices + 5 chapitres livrés). Brief archivé hors dépôt. Validation **locale** (Java 25 + Maven) + CI : 22/22 solutions, lint 0 erreur.
  - ✅ **Volet chapitres livré** (PR #16 mergée) : `_category_.json` + 5 chapitres (2.1 Tableaux 1D, 2.2 Tableaux 2D, 2.3 Chaînes, 2.4 Méthodes, 2.5 Récursivité) + liaison module 1→2. Build Docusaurus vert. Spec et plan archivés hors dépôt.
  - ✅ **Volet exercices livré** (cycles séparés, 1 sous-groupe = 1 PR) : 2.1 Tableaux, 2.2 Chaînes, 2.3 Méthodes, 2.4 Récursivité. **Brief de démarrage** archivé hors dépôt. Décisions transverses figées (spec chapitres §10) : tout dans `main` pour 2.1/2.2, méthodes `static` testées par retour pour 2.3/2.4, récursivité imposée par consigne, refactor 2.3.3 basé sur 1.2.2.
    - ✅ **2.1 Tableaux** (PR #18) : `min-max-moyenne`, `inversion`, `recherche-lineaire`, `rotation-matrice`. Spec et plan archivés hors dépôt.
    - ✅ **2.2 Chaînes** (PR #19) : `palindrome`, `comptage-occurrences`, `ascii-art`. Saisie clavier + test sur sortie ; solutions validées en local (`valider-solutions` + lint 0 erreur). Spec archivée hors dépôt.
    - ✅ **2.3 Méthodes** (PR #20) : `biblio-maths`, `surcharge`, `refactor-calculs-geometriques`. Méthodes `static` à signature imposée, **testées par valeur de retour** (AssertJ direct). Spec archivée hors dépôt.
    - ✅ **2.4 Récursivité** : `factorielle-puissance`, `parcours-matrice-recursif`. Récursivité imposée (critère formateur `respect-consignes`), testée par valeur de retour. Spec archivée hors dépôt.
- **#56 — Mode standalone (ZIP autonome stagiaire)** — **FAITE** (insérée avant le module 3). ZIP hors-ligne complet : site de cours servi en local **in-process** (`CourseSiteServer` via `SimpleFileServer`, option `--site` du REPL), **MinGit** épinglé + SHA-256 vérifié, JDK portable, moulinette pré-compilée, launcher Windows unique. Spec, plan et brief archivés hors dépôt. Décisions figées : Maven/`.m2` hors scope (la moulinette grade via javac + JUnit ConsoleLauncher) ; Windows d'abord. Doc des **deux modes** : nouvelle page `courses/docs/lancer-la-piscine.md` + `README.md` + `docs/piscine-stagiaire.md` + `CONTRIBUTING.md`. Reste : **validation sur machine Windows propre** (run formateur, §2.D du brief).
- **#19 — Module 3 (POO)** (10 chapitres, 14 exercices, 4 sous-groupes) → débloque le **Projet binôme #1**. ✅ **TERMINÉ** (chapitres + exercices livrés ; reste à rédiger la fiche du Projet binôme #1 dans un cycle dédié). Brief, spec et plan archivés hors dépôt.
...
### #23 — Projet binôme #1 — Mini-domaine OO (Bureau)
**Pré-requis** : #19
Voir `referentiel.md` §5. ✅ **Fiche livrée** dans `exercises/projets-binome/projet-1-mini-domaine/` (sujet, consignes, grille, métadonnées). Conception (spec + plan) archivée hors dépôt.

### #24 — Projet binôme #2 — App + persistance fichier (TaskManager CLI)
**Pré-requis** : #21
Voir `referentiel.md` §5. ✅ **Fiche livrée** dans `exercises/projets-binome/projet-2-persistance/` (sujet, consignes, grille, métadonnées). Conception (spec + plan) archivée hors dépôt.

### #25 — Projet binôme #3 — Mini-moulinette testée (final)
**Pré-requis** : #22
**Statut** : ✅ **Fiche livrée** (branche `feature/projet-binome-3`, **PR à ouvrir/merger**) dans `exercises/projets-binome/projet-3-mini-moulinette/` (sujet, consignes, grille, métadonnées). **Mini-moulinette pédagogique** : pipeline compiler→exécuter→rapport sur un `Operation.java` à contrat fixe (`appliquer(int):int`), cas lus depuis `cas.csv` (à la main), **tests JUnit ≥70 % couverture** + **workflow Git branches/PR** exigés (premier projet). Synthèse OO/persistance/tests/Git. Conception (spec) archivée hors dépôt. **→ Phase 3 TERMINÉE** (modules 2-6 + 3 projets binôme).

---

## Phase 4 — Production & déploiement en entreprise (priorité BASSE pour l'instant)
...
### #27 — Auth LDAP / AD d'entreprise
**Pré-requis** : interlocuteur infra d'entreprise identifié.

### #28 — Mirror Maven interne (Nexus / Artifactory)
**Pourquoi** : pas d'accès direct à Maven Central depuis l'environnement d'entreprise.
...
### #31 — Conformité sécurité interne
**Pré-requis** : confirmation du responsable du projet de l'entreprise.
...
### #33 — Branding Piscine Java
**Statut** : À faire
**Priorité** : Basse
Remplacer les assets Docusaurus par défaut (favicon, logo, social card) par les visuels Piscine Java.
...
**Statut** : **Faite** — [PR #1](https://github.com/Benjamin-Curlier/piscine-java/pull/1) mergée (merge commit `c86e098`), branche supprimée. La target-MVP est exécutée depuis `feature/target-mvp-checkers` (issue de `main`).
...
**Priorité** : Basse
**Pré-requis** : #10
**Pourquoi** : versionner `.mvn/wrapper/maven-wrapper.jar` rend `./mvnw` fonctionnel **offline**, sans dépendre du réseau (pratique standard pour des builds reproductibles, pertinent vu le réseau restreint côté formateur et l'environnement d'entreprise cible).
