...
public final class StyleChecker implements Checker {
    ...
    public static Path extractBundledConfig() {
        try (InputStream in = StyleChecker.class.getResourceAsStream("/checkstyle-piscine.xml")) {
            if (in == null) throw new IllegalStateException("checkstyle-piscine.xml introuvable dans les resources");
            Path tmp = Files.createTempFile("checkstyle-piscine", ".xml");
