...
public final class LocalWorkspaceInitializer implements WorkspaceInitializer {
    ...
    public Workspace init(InitRequest req) {
        try {
            ...
            check(git.run(repoRoot, List.of("config", "user.email", req.nom() + "@piscine.piscine")), "config email");
            ...
            Files.writeString(repoRoot.resolve("README.md"),
                "# Piscine Java — workspace de " + req.nom() + "\n\n"
                + "Voir docs/piscine-stagiaire.md dans le repo Piscine Java.\n");
